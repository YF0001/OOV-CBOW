# -*- coding: utf-8 -*-
"""
Created on Sun Aug 13 14:24:42 2017
...
class Word2VecIterator(object):
    ...
    def __init__(self, corpus, n_neg, batch_size, window, min_count,
                 model=None):
        self.corpus = word2vec.Text8Corpus(corpus)
        self.n_neg = n_neg
        ...
        if not model:
            ...
            for word in model.wv.vocab:
                self.word_counts[word] = model.wv.vocab[word].count
                self.word2index[word] = model.wv.vocab[word].index

            self.word_index = sorted(list(self.word2index.keys()),
                                     key=lambda x: model.wv.vocab[x].index)
            self.make_neg_table()
...
#            contents = []
    ...
    #                contents.append(item[0])
            ...
            r = numpy.zeros((batch_size, self.window*2), dtype=numpy.int32)
            for i, (forward, backward,context) in enumerate(zip(content_forward,
                                                        content_backward,contexts)):
                l1[i, :len(forward)] = forward
                l2[i, :len(backward)] = backward
                r[i,:]=context
#            l = numpy.array(contents, dtype=numpy.int32)
            w = numpy.array(words, dtype=numpy.int32)
            labels = numpy.array(labels)
            weights = numpy.array(weights)
            return ([l1, l2, r, w],  # these are the inputs
        ...
        for line in self.corpus:
            ...
            for pos, word in enumerate(line):
                ...
                start = max(0, pos - self.window)
                all_context=[]
                for pos2, word2 in enumerate(line[start:(pos + self.window+1)], start):
                    if pos2 != pos:
                        # add both words to the batch with label 1
                        all_context.append(self.word2index[word2])
                if len(all_context)!=  int(self.window)*2:                  
                    continue
                buffer[current_key] = (word_chars, all_context,[self.word2index[word]], 1., 1.)
                current_key += 1
                neg_samples = []
                ...
                while len(neg_samples) < self.n_neg:
                    ...
                    if word2 not in [self.word2index[word]] + neg_samples+all_context:
                        if self.model.similarity(self.model.wv.index2word[word2],word)<0.1:
                            neg_samples.append(word2)
                            buffer[current_key] = (word_chars, all_context,[word2], 0., 1.)
                            current_key += 1
...
class MENEvaluator(Callback):
    ...
    def on_train_begin(self, logs={}):
        ...
        with open('D:\\final_project\\code\\learn\\mvlsa-master\\mvlsa-master\\res\\word_sim\\men.README\\MEN_dataset_lemma_form.dev', 'r') as f:
            for line in f:
                line = line.split()
